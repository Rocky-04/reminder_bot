        ...
        for reminder in reminders:
            if reminder.next_data <= datetime.now():
                ...
                if reminder.periodicity.interval == 0:
                    next_data == 'Repeated only once'
                    notification_manager.delete(id=reminder.id, user_id=reminder.user)
                else:
                    next_data = reminder.next_data + timedelta(
                        minutes=reminder.periodicity.interval)
